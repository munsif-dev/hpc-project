...
that accuracy can be compared directly. In the final 80-epoch run, the
serial baseline reaches test $\mathrm{Q}_3 = 59.28\,\%$ and acts as the
reference target for every parallel variant. We measure per-epoch
wall-clock time as a function of
...
\begin{figure}[H]
  ...
  \caption{Preprocessing workflow exported from the draw.io project
  diagram. Raw CB513 files are parsed, mapped from eight DSSP classes to
  three Q3 classes, converted to sliding-window one-hot features, split,
  and persisted as binary tensors for all training variants.}
...
\begin{figure}[H]
  ...
  \caption{Serial training-loop workflow exported from the draw.io project
  diagram. Each mini-batch performs forward propagation, back-propagation,
  one SGD update, validation logging, and final test evaluation.}
...
\begin{figure}[H]
  ...
  \caption{OpenMP shared-memory workflow exported from the draw.io project
  diagram. Model weights are shared and read-only during the parallel
  region; each thread writes only to its own private gradient buffer before
  the master performs one reduction and SGD update.}
  \label{fig:omp}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{04_pthreads_master_worker.png}
  \caption{Pthreads master/worker workflow exported from the draw.io
  project diagram. Workers live for the entire run; two barriers gate each
  mini-batch, and the master performs the gradient reduction and SGD
  update after all workers finish their slices.}
  \label{fig:pth}
\end{figure}
...
multiple MPI ranks with separate address spaces. It can run across
physical machines, but the final measurements in this report use local
multi-rank execution on the HPC host. All ranks load the full dataset and
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{05_mpi_distributed_memory.png}
  \caption{MPI distributed-memory workflow exported from the draw.io
  project diagram. Each rank owns a private model copy and local gradient;
  one packed \texttt{MPI\_Allreduce(SUM)} synchronises the full gradient
  before every rank applies the same SGD update.}
  \label{fig:mpi}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{06_hybrid_mpi_openmp.png}
  \caption{Hybrid MPI+OpenMP workflow exported from the draw.io project
  diagram. MPI performs the outer distributed-memory decomposition across
  ranks, OpenMP performs the inner shared-memory decomposition within each
  rank, and only the rank-local reduced gradients enter the MPI
  \texttt{Allreduce}.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.86\textwidth]{07_cuda_gpu_workflow.png}
  \caption{CUDA GPU workflow exported from the draw.io project diagram.
  The host shuffles indices and launches kernels, while the training data,
  model tensors, cuBLAS operations, custom kernels, and in-place SGD
...
\begin{table}[H]
  ...
  \begin{tabular}{llrrr}
    \toprule
    variant   & config            & test $\mathrm{Q}_3$ & $\Delta$ vs serial & total time \\
    \midrule
...
From \texttt{plots/timing\_summary.csv}, the 20-epoch timing sweep gives:
\begin{itemize}[noitemsep,topsep=2pt]
...
CUDA delivers the largest speedup while preserving accuracy at the
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  absolute $\mathrm{Q}_3$ substantially beyond the one-hot feature
  baseline, into the 70\,+\,\% range reported in the paper.
\end{itemize}
...
\texttt{docs/diagrams/hpc\_workflows.drawio}. The exported PNG versions
included in this report are stored under \texttt{docs/diagrams/png/}.
